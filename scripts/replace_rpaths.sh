#!/bin/bash
#
# For each Qt framework dylib, replaces the @rpath with @executable_path
# This is necessary if Qt / Xcode builds the application so that all libraries
# are trying to find other libraries using rpath
# Run in the frameworks directory, ie.
# cd ~/Products/Debug/Products/Delicode_NI_mate.app/Contents/Frameworks
# cp ~/Products/Installers/Mac/replace_rpaths.sh ~/Products/Debug/Products/Delicode_NI_mate.app/Contents/Frameworks
...
for FILE in $(find . -path './[Q][t]*.framework/Versions/5/*' ! -name '*.plist' ! -name 'Resources')
do
...
for FILE in $(find . -path './[Q][t]*.framework/Versions/1/*' ! -name '*.plist' ! -name 'Resources')
do
    echo "Handling file  " $FILE
    LIBNAME=$(echo $FILE | rev | cut -d'/' -f1 | rev)
